...
<div class="container">
    ...
    <form wire:submit.prevent="openSaveConfirmation" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-3 px-1 py-1">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" class="sr-only peer" :checked="$wire.orderType === 'deliver'" @change="$wire.set('orderType', $event.target.checked ? 'deliver' : 'walk_in')">
                        ...
                        <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300 transition-all duration-300 flex items-center">
                            <i class="mr-1 transition-all duration-300" :class="$wire.orderType === 'deliver' ? 'fas fa-truck text-blue-500' : 'fas fa-walking text-orange-500'"></i>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div x-data="{
                            ...
                            <button type="button"
                                ...
                                <span class="truncate">{{ optional($this->selectedEmployee)->name ?? __('Select delivery person') }}</span>
            ...
            @if($showProductForm)
                <div class="mb-5 rounded-2xl border border-blue-200 dark:border-blue-900/40 bg-blue-50/70 dark:bg-blue-900/10 p-4">
                    ...
                    <div class="flex justify-end mt-4">
                        <button type="button" wire:click="createProduct" wire:loading.attr="disabled"
                            class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition">
        ...
        <div class="flex justify-center gap-3 w-full">
            <a href="{{ route('orders') }}" wire:navigate class="inline-flex items-center justify-center w-full gap-2 px-4 py-3 rounded-lg bg-zinc-100 dark:bg-zinc-700 text-sm font-semibold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-200 dark:hover:bg-zinc-600 transition">
                <i class="fas fa-times mr-1"></i>
                <span>{{ __('Cancel') }}</span>
            </a>
            <button type="submit"
                class="inline-flex items-center justify-center w-full gap-2 px-6 py-4 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 active:scale-95 transition-all shadow-md shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed">
                <i class="fas fa-save mr-1"></i>
                <span>{{ __('Save Record') }}</span>
            </button>
        </div>
    </form>
    ...
    @include('livewire.partials.orders.modal.confirm', [
        'confirmData' => [
            ...
            'reviewDateTime' => \Carbon\Carbon::parse($saleDate)->locale(app()->getLocale())->isoFormat('LLLL'),
    ...
    {{-- Loading Overlay --}}
    @include('livewire.partials.loading-overlay', ['wireTarget' => 'createOrder,createProduct,selectProduct,selectCustomer,selectEmployee,addOrderItem,removeOrderItem,openProductForm,closeProductForm,saveSalesRecord'])
