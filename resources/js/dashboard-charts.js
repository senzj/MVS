...
// Helper functions for currency formatting (sourced from config/storeconfig.php)
function getCurrencySymbol() {
  return window.__currencySymbol || '₱';
}

function formatCurrency(value) {
  return `${getCurrencySymbol()}${Number(value).toLocaleString()}`;
}
...
export function initDashboardCharts(payload) {
    ...
    if (salesVsProfitData.labels?.length) {
        makeChart('salesVsProfit', document.getElementById('salesVsProfitChart'), {
            type: 'line',
            data: {
                labels: translateDateLabels(salesVsProfitData.labels, 'month'),
                datasets: [
                    {
                    ...
                    {
                        label: i18n.netloss || 'Net Loss',
                        data: salesVsProfitData.netloss,
                        borderColor: '#f43f5e',
                    ...
                    {
                        label: i18n.gross_profit || 'Profit',
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        title: {
                            display: true,
                            text: i18n.amount_currency || 'Amount'
                        },
                        ticks: {
                    ...
                    y1: {
                        ...
                        title: {
                            display: true,
                            text: i18n.num_orders || 'Number of Orders'
                        },
                        grid: {
    ...
    if (busiestMetrics) {
        try {
            const byYear = busiestMetrics.by_year || {
                labels: [],
                orders: [],
                profit: [],
                summary: {}
            ...
            const byMonth = busiestMetrics.by_month || {
                labels: [],
                orders: [],
                profit: [],
                summary: {}
            ...
            const byWeekday = busiestMetrics.by_weekday || {
                labels: [],
                orders: [],
                profit: [],
                summary: {}
            ...
            const byHour = busiestMetrics.by_hour || {
                labels: [],
                orders: [],
                profit: [],
                summary: {}
                ...
                makeChart('busiestByYear', document.getElementById('busiestByYearChart'), {
                    ...
                    data: {
                        ...
                        datasets: [
                            ...
                            {
                                label: i18n.estimated_profit || `${i18n.estimated_profit} (${getCurrencySymbol()})`,
                                data: byYear.profit,
                        ...
                        scales: {
                            y: {
                                ...
                                grid: {
                                    ...
                                    display: true,
                                    text: i18n.amount_currency || `Amount (${getCurrencySymbol()})`,
                                }
                            }
                        }
                    },
                    ...
                    options: {
                        ...
                        scales: {
                            ...
                            y1: {
                                ...
                                ticks: {
                                    callback: (v)=> (getCurrencySymbol() + Number(v).toLocaleString())
                                },
                            },
                        },
                    },
    ...
    if (busiestMetrics && busiestMetrics.by_year) {
        ...
        if (ysum.most_orders)
            setText('year-summary-most-orders', `${i18n.most_busy || 'Most orders'}: ${ysum.most_orders.label} (${ysum.most_orders.value})`);

        if (ysum.most_profit)
            setText('year-summary-most-profit', `${i18n.most_profitable || 'Most profit'}: ${ysum.most_profit.label} (${ysum.most_profit.value})`);

        const msum = s.by_month.summary || {};
        if (msum.most_orders)
            setText('month-summary-most-orders', `${i18n.most_busy || 'Most orders'}: ${msum.most_orders.label} (${msum.most_orders.value})`);

        if (msum.most_profit)
            setText('month-summary-most-profit', `${i18n.most_profitable || 'Most profit'}: ${msum.most_profit.label} (${msum.most_profit.value})`);

        const wsum = s.by_weekday.summary || {};
        if (wsum.most_orders)
            setText('weekday-summary-most-orders', `${i18n.most_busy || 'Most orders'}: ${wsum.most_orders.label} (${wsum.most_orders.value})`);

        if (wsum.most_profit)
            setText('weekday-summary-most-profit', `${i18n.most_profitable || 'Most profit'}: ${wsum.most_profit.label} (${wsum.most_profit.value})`);

        const hsum = s.by_hour.summary || {};
        if (hsum.most_orders)
            setText('hour-summary-most-orders', `${i18n.most_busy || 'Most orders'}: ${hsum.most_orders.label} (${hsum.most_orders.value})`);

        if (hsum.most_profit)
            setText('hour-summary-most-profit', `${i18n.most_profitable || 'Most profit'}: ${hsum.most_profit.label} (${hsum.most_profit.value})`);
    }
    ...
    if (ordersByDayData.labels?.length) {
        makeChart('ordersByDay', document.getElementById('ordersByDayChart'), {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: i18n.current_week || 'Current Week',
                    ...
                    {
                        label: i18n.previous_week || 'Previous Week',
    ...
    if (monthlyTrendsData.labels?.length) {
        ...
        data: {
            ...
            datasets: [
                {
                    label: i18n.monthly_sales || `Monthly Sales (${getCurrencySymbol()})`,
                ...
                {
                    label: i18n.monthly_orders || 'Monthly Orders',
        ...
        options: {
            ...
            scales: {
                y: {
                    ...
                    title: {
                        display: true,
                        text: i18n.sales_amount_currency || `Sales Amount (${getCurrencySymbol()})`
                    }
                },
                y1:{
                    type: 'linear',
    ...
    if (categoryBreakdownData.labels?.length) {
        makeChart('categoryBreakdown', document.getElementById('categoryBreakdownChart'), {
            ...
            data: {
                ...
                datasets: [{
                    label: i18n.sales || `Sales (${getCurrencySymbol()})`,
